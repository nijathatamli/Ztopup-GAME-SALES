...
$PG_CONNECT_TIMEOUT = (int) (getenv('PGCONNECT_TIMEOUT') ?: 5);

$sslDsn = ($SSL_MODE && strtolower($SSL_MODE) === 'require') ? ';sslmode=require' : '';
$timeoutDsn = $PG_CONNECT_TIMEOUT > 0 ? ";connect_timeout={$PG_CONNECT_TIMEOUT}" : '';
$dsn = "pgsql:host={$DB_HOST};port={$DB_PORT};dbname={$DB_NAME}{$sslDsn}{$timeoutDsn}";

try {
    $pdo = new PDO($dsn, $DB_USER, $DB_PASSWORD, [
        PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
        PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC,
        PDO::ATTR_TIMEOUT => max(1, $PG_CONNECT_TIMEOUT),
    ]);
} catch (Throwable $e) {
...
function ensure_schema(PDO $pdo): void {
    ...
    // Deposit requests (receipt uploads)
    $pdo->exec(<<<SQL
    ...
    SQL);
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_deposit_user ON deposit_requests(user_id)");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_deposit_status ON deposit_requests(status)");
    try { $pdo->exec("ALTER TABLE deposit_requests ADD CONSTRAINT fk_deposit_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE"); } catch (Throwable $e) {}
...
function csrf_check(): void {
    ...
    if (!is_string($token) || $token === '' || empty($_SESSION['csrf']) || !hash_equals($_SESSION['csrf'], $token)) {
        ...
        exit('Etibarsız CSRF tokeni. Səhifəni yeniləyin və yenidən cəhd edin.');
    }
}

// Absolute path to the shared uploads directory (outside admin docroot)
const UPLOADS_DIR = __DIR__ . '/../uploads/receipts';
