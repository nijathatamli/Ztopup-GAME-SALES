...
<link rel="icon" type="image/png" href="/assets/ztopup-logo-gold.png">
<link rel="alternate icon" href="/assets/ztopup-logo-gold.png">
...
async function ensureAdminSchema(pool){
  ...
  if (!await hasCol('admins', 'role')) await pool.query("ALTER TABLE admins ADD COLUMN role TEXT NOT NULL DEFAULT 'admin'");
  if (!await hasCol('admins', 'last_login_at')) await pool.query('ALTER TABLE admins ADD COLUMN last_login_at TIMESTAMP NULL');
  if (!await hasCol('admins', 'last_login_ip')) await pool.query('ALTER TABLE admins ADD COLUMN last_login_ip VARCHAR(45) NULL');
  if (!await hasCol('admins', 'username')) {
    await pool.query("ALTER TABLE admins ADD COLUMN username VARCHAR(100) NOT NULL DEFAULT 'admin'");
    await pool.query('ALTER TABLE admins ADD CONSTRAINT admins_username_key UNIQUE(username)');
  }
  ...
  if (!await hasCol('admin_roles', 'role')) await pool.query('ALTER TABLE admin_roles ADD COLUMN role TEXT');
  if (!await hasCol('admin_roles', 'name')) await pool.query('ALTER TABLE admin_roles ADD COLUMN name TEXT');
  if (!await hasCol('admin_roles', 'description')) await pool.query('ALTER TABLE admin_roles ADD COLUMN description TEXT');
  
  if (await hasCol('admin_roles', 'id') && await hasCol('admin_roles', 'name')) {
    await pool.query("UPDATE admin_roles SET role = COALESCE(role, name, 'role_' || id::text) WHERE role IS NULL");
  } else if (await hasCol('admin_roles', 'name')) {
    await pool.query("UPDATE admin_roles SET role = COALESCE(role, name, 'legacy') WHERE role IS NULL");
  } else {
    await pool.query("UPDATE admin_roles SET role = 'legacy' WHERE role IS NULL");
  }
  await pool.query("UPDATE admin_roles SET name = COALESCE(name, role, 'Legacy Role') WHERE name IS NULL");
  
  try { await pool.query('CREATE UNIQUE INDEX admin_roles_role_key ON admin_roles(role)'); } catch(e){}
  ...
  if (!await hasCol('admin_permissions', 'role')) await pool.query('ALTER TABLE admin_permissions ADD COLUMN role TEXT');
  
  if (await hasCol('admin_permissions', 'role_id') && await hasCol('admin_roles', 'id')) {
  ...
  if (!await hasCol('admin_permissions', 'created_at')) await pool.query('ALTER TABLE admin_permissions ADD COLUMN created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP');
  
  try { await pool.query('CREATE UNIQUE INDEX admin_permissions_role_permission_key ON admin_permissions(role, permission)'); } catch(e){}
  ...
  if (!await hasCol('admin_sessions', 'token')) await pool.query('ALTER TABLE admin_sessions ADD COLUMN token VARCHAR(255)');
  if (!await hasCol('admin_sessions', 'refresh_token_hash')) {
    await pool.query('ALTER TABLE admin_sessions ADD COLUMN refresh_token_hash TEXT');
    await pool.query("UPDATE admin_sessions SET refresh_token_hash = COALESCE(token, 'legacy_refresh_token') WHERE refresh_token_hash IS NULL");
  ...
  if (!await hasCol('admin_sessions', 'csrf_token')) await pool.query("ALTER TABLE admin_sessions ADD COLUMN csrf_token VARCHAR(255) NOT NULL DEFAULT ''");
  if (!await hasCol('admin_sessions', 'ip_address')) await pool.query('ALTER TABLE admin_sessions ADD COLUMN ip_address VARCHAR(45)');
  if (!await hasCol('admin_sessions', 'user_agent')) await pool.query('ALTER TABLE admin_sessions ADD COLUMN user_agent TEXT');
  if (!await hasCol('admin_sessions', 'created_at')) await pool.query('ALTER TABLE admin_sessions ADD COLUMN created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP');
  
  try { await pool.query('CREATE INDEX idx_admin_sessions_token ON admin_sessions(token)'); } catch(e){}
  try { await pool.query('CREATE INDEX idx_admin_sessions_expires ON admin_sessions(expires_at)'); } catch(e){}

  // 5. audit_logs table (ensure columns exist before use)
  ...
  if (!await hasCol('audit_logs', 'admin_username')) await pool.query('ALTER TABLE audit_logs ADD COLUMN admin_username TEXT NULL');

  // Seed default roles and permissions
  const roles = [
    { role: 'super_admin', description: 'Full system access', name: 'Super Admin' },
    { role: 'admin', description: 'Standard admin access', name: 'Admin' },
    { role: 'moderator', description: 'Order and user management', name: 'Moderator' },
    { role: 'support', description: 'Customer support access', name: 'Support' }
  ];
  ...
  for (const p of permissions) {
    if (await hasCol('admin_permissions', 'role_id')) {
      ...
      await pool.query(
        `INSERT INTO admin_permissions(role, permission, created_at)
         VALUES ($1, $2, NOW())
  ...
  try { await pool.query('CREATE INDEX idx_balance_user ON balance_requests(user_id)'); } catch(e){}
  try { await pool.query('CREATE INDEX idx_balance_status ON balance_requests(status)'); } catch(e){}
  ...
  try {
    ...
    console.error('[Admin] Migration warning:', e.message); 
  }
...
async function rLogin(req,res,pool){
  ...
  if(req.method==='POST'){if(!checkLoginLimit(clientIp))return sendHtml(res,429,'<h1>Çox sayda cəhdi. Bir az gözləyin.</h1>');try{const body=await readBody(req);const id=String(body.identifier||'').trim().toLowerCase();const pw=String(body.password||'');const a=await findAdmin(pool,id);if(!a||!verifyPw(pw,a.password_hash)){auditLog(pool,'admin_login_failed',{admin:{id:'unknown',username:'unknown'},req,targetId:id,meta:{identifier:id}});const t=csrfToken();setCookie(res,'admin_csrf',t,3600);const html=`<!doctype html><html lang="az"><head><meta charset="utf-8"/><style>body{margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;background:#0b0b12;color:#fff;font-family:Rajdhani,system-ui}.card{width:100%;max-width:420px;background:linear-gradient(180deg,rgba(255,255,255,.06),rgba(255,255,255,.03));border:1px solid rgba(255,255,255,.12);border-radius:16px;padding:24px}h1{font-size:22px;margin:0 0 16px}label{display:block;font-size:13px;color:#c9c9d1;margin:10px 0 6px}input{width:100%;background:#141427;border:1px solid rgba(255,255,255,.1);color:#fff;border-radius:10px;padding:10px 12px;outline:none;box-sizing:border-box}button{width:100%;margin-top:16px;background:#6c4df4;border:none;color:#fff;padding:12px 14px;border-radius:12px;font-weight:800;cursor:pointer}.error{margin-top:10px;background:rgba(255,99,71,.1);border:1px solid rgba(255,99,71,.3);padding:10px;border-radius:10px;color:#ffb3a7}</style></head><body><div class="card"><h1>Admin Girişi</h1><div class="error">Giriş məlumatları səhvdir</div><form method="post"><input type="hidden" name="csrf" value="${esc(t)}"/><label>Email və ya İstifadəçi Adı</label><input type="text" name="identifier" required value="${esc(id)}"/><label>Şifrə</label><input type="password" name="password" required/><button type="submit">Daxil ol</button></form></div></body></html>`;return sendHtml(res,200,html);}
    ...
    return;
  }catch(e){console.error('[Admin Login Error]',e);return sendHtml(res,500,'<h1>Server xətası: '+esc(e.message)+'</h1>');}}
...
async function rDeposits(req,res,pool){
  ...
  if(req.method==='POST'){const body=await readBody(req);const c=parseCookies(req);if(!body.csrf||body.csrf!==a.csrfToken)return sendHtml(res,419,'<h1>Etibarsız CSRF tokeni.</h1>');const id=String(body.id||''),action=String(body.action||''),note=String(body.note||'').trim();if(id&&(action==='approve'||action==='reject')){const client=await pool.connect();try{await client.query('BEGIN');const r=await client.query('SELECT*FROM deposit_requests WHERE id=$1 LIMIT 1 FOR UPDATE',[id]);const row=r.rows[0];if(!row)throw new Error('Sorğu tapılmadı');if(String(row.status).toLowerCase()!=='pending')throw new Error('Bu sorğu artıq emal edilib');if(action==='approve'){const amt=parseFloat(body.amount||0);if(!isFinite(amt)||amt<=0)throw new Error('Düzgün məbləğ daxil edin');const ubr=await client.query('SELECT balance FROM users WHERE id=$1',[row.user_id]);const prevBal=Number((ubr.rows[0]&&ubr.rows[0].balance)||0);const refNum='DEP-'+Date.now()+'-'+crypto.randomBytes(3).toString('hex').toUpperCase();await client.query('UPDATE users SET balance=balance+$1 WHERE id=$2',[amt,row.user_id]);await client.query("UPDATE deposit_requests SET status='approved',approved_at=NOW(),admin_note=$1,admin_id=$2,admin_username=$3,approved_amount=$4,prev_balance=$5,new_balance=$6,reference_number=$7,updated_at=NOW() WHERE id=$8",[note||null,a.id,a.username||a.id,amt,prevBal,prevBal+amt,refNum,id]);const tid=crypto.randomBytes(16).toString('hex');await client.query('INSERT INTO transactions(id,user_id,amount,type,status,ref)VALUES($1,$2,$3,$4,$5,$6)',[tid,row.user_id,amt,'credit','approved','Deposit approved by admin (request '+row.id+')']);await createNotification(client,row.user_id,'Balans artırıldı','Hesabınıza '+amt.toFixed(2)+' ₼ əlavə olundu.','balance');ssePushState(row.user_id);recalcMembership(row.user_id);flash={type:'ok',msg:'Depozit təsdiqləndi və '+amt.toFixed(2)+' ₼ balansa əlavə olundu'};auditLog(pool,'admin_deposit_approved',{admin:a,req,targetType:'deposit_request',targetId:id,oldValue:{status:'pending'},newValue:{status:'approved',amount:amt},meta:{userId:row.user_id}});}else{await client.query("UPDATE deposit_requests SET status='rejected',rejected_at=NOW(),admin_note=$1,rejection_reason=$1,admin_id=$2,admin_username=$3,updated_at=NOW() WHERE id=$4",[note||null,a.id,a.username||a.id,id]);flash={type:'ok',msg:'Depozit sorğusu rədd edildi'};auditLog(pool,'admin_deposit_rejected',{admin:a,req,targetType:'deposit_request',targetId:id,oldValue:{status:'pending'},newValue:{status:'rejected'},meta:{userId:row.user_id}});}await client.query('COMMIT');}catch(e){await client.query('ROLLBACK').catch(()=>{});flash={type:'bad',msg:'Xəta: '+e.message};}finally{client.release();}}}
  const status=url.searchParams.get('status')||'';const q=String(url.searchParams.get('q')||'').trim();const dateFrom=url.searchParams.get('from')||'';const dateTo=url.searchParams.get('to')||'';let sql='SELECT d.*,u.username,u.email,u.first_name,u.last_name FROM deposit_requests d LEFT JOIN users u ON u.id=d.user_id WHERE 1=1',params=[];if(status){sql+=' AND LOWER(d.status)=LOWER($'+(params.length+1)+')';params.push(status);}if(q){sql+=' AND (LOWER(u.username) LIKE $'+(params.length+1)+' OR d.id LIKE $'+(params.length+1)+' OR d.reference_number LIKE $'+(params.length+1)+')';params.push('%'+q.toLowerCase()+'%');}if(dateFrom){sql+=' AND d.created_at >= $'+(params.length+1);params.push(dateFrom);}if(dateTo){sql+=' AND d.created_at <= $'+(params.length+1)+'::date+1';params.push(dateTo);}sql+=' ORDER BY d.created_at DESC LIMIT 500';const{rows}=await pool.query(sql,params);const statsRes=await pool.query('SELECT status,COUNT(*)::int AS c,COALESCE(SUM(approved_amount),SUM(requested_amount),0)::numeric AS total FROM deposit_requests GROUP BY status');const stats={};for(const s of statsRes.rows)stats[s.status]={count:s.c,total:Number(s.total)};const allCount=statsRes.rows.reduce((a,b)=>a+b.c,0);
  const cs=a.csrfToken;setCookie(res,'admin_csrf',cs,ADMIN_SESSION_MAX_AGE_SECONDS);
  let tr='';if(!rows.length)tr=`<tr><td colspan="12" style="color:#9a9aa6;padding:20px 0;text-align:center">Depozit tapılmadı.</td></tr>`;
  for(const r of rows){const st=String(r.status).toLowerCase();const img='/admin/receipt?file='+encodeURIComponent(r.receipt_image);const acts=st==='pending'?`<div class="actions"><button class="btn-approve" onclick="openApprove('${esc(r.id)}','${esc(r.username||'')}','${esc(parseFloat(r.requested_amount||0).toFixed(2))}')">Təsdiqlə</button><button class="btn-reject" onclick="openReject('${esc(r.id)}')">Rədd et</button></div>`:`<span style="color:#9a9aa6">—</span>`;const processedAt=r.approved_at||r.rejected_at;const approvedAmt=r.approved_amount?'₼ '+parseFloat(r.approved_amount).toFixed(2):'—';tr+=`<tr><td style="font-size:11px;color:#9a9aa6;font-family:monospace">${esc((r.id||'').slice(0,8))}</td><td>${esc(r.username||'—')}<br/><span style="font-size:11px;color:#9a9aa6">${esc(r.email||'')}</span></td><td><img class="thumb" src="${esc(img)}" onclick="viewImg('${esc(img)}','${esc(r.reference_number||'')}')"/></td><td>₼ ${parseFloat(r.requested_amount||0).toFixed(2)}<br/><span style="font-size:11px;color:${st==='approved'?'#a3ffcf':'#9a9aa6'}">${approvedAmt}</span></td><td><span class="badge ${st==='approved'?'b-approved':(st==='rejected'?'b-rejected':'b-pending')}">${esc(r.status)}</span></td><td style="font-size:11px;font-family:monospace">${esc(String(r.created_at).slice(0,16))}</td><td style="font-size:11px">${processedAt?esc(String(processedAt).slice(0,16)):'—'}</td><td style="font-size:11px;color:#9a9aa6">${esc(r.admin_username||'—')}</td><td style="font-size:11px;font-family:monospace;color:#7eb8f7">${esc(r.reference_number||'—')}</td><td style="max-width:160px;font-size:11px;color:#c9c9d1">${esc(r.admin_note||'')}</td><td>${acts}</td></tr>`;}
  const statsBar=`<div class="card" style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;padding:10px 14px"><span style="font-size:13px;font-weight:700">Cəmi: ${allCount}</span><span style="color:#ffd24d">⏳ Gözləmədə: ${(stats.pending&&stats.pending.count)||0}</span><span style="color:#a3ffcf">✓ Təsdiqlənmiş: ${(stats.approved&&stats.approved.count)||0} — ₼ ${((stats.approved&&stats.approved.total)||0).toFixed(2)}</span><span style="color:#ffb3a7">✗ Rədd: ${(stats.rejected&&stats.rejected.count)||0}</span></div>`;const content=statsBar+`<form method="get" class="card" style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end"><div style="display:flex;flex-direction:column;gap:4px"><label style="font-size:11px">Status</label><select name="status"><option value="" ${status===''?'selected':''}>Hamısı</option><option value="pending" ${status==='pending'?'selected':''}>Gözləmədə</option><option value="approved" ${status==='approved'?'selected':''}>Təsdiqlənmiş</option><option value="rejected" ${status==='rejected'?'selected':''}>Rədd edilmiş</option></select></div><div style="display:flex;flex-direction:column;gap:4px"><label style="font-size:11px">Axtar (user/ID/ref)</label><input name="q" value="${esc(q)}" placeholder="Axtar..."/></div><div style="display:flex;flex-direction:column;gap:4px"><label style="font-size:11px">Tarixdən</label><input type="date" name="from" value="${esc(dateFrom)}"/></div><div style="display:flex;flex-direction:column;gap:4px"><label style="font-size:11px">Tarixə</label><input type="date" name="to" value="${esc(dateTo)}"/></div><button type="submit">Filtrlə</button><a href="/admin/deposits" style="padding:8px 12px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:8px;color:#fff;text-decoration:none;font-size:13px">Sıfırla</a></form><div class="card" style="overflow-x:auto"><table><thead><tr><th>ID</th><th>İstifadəçi</th><th>Qəbz</th><th>Məbləğ</th><th>Status</th><th>Yaradılıb</th><th>Emal Tarixi</th><th>Admin</th><th>Ref №</th><th>Qeyd</th><th>Əməliyyat</th></tr></thead><tbody>${tr}</tbody></table></div>
...
<dialog id="im"><img id="is" src="" style="max-width:100%;max-height:68vh;border-radius:10px;display:block;margin:0 auto"/><div style="margin-top:8px;font-size:11px;color:#9a9aa6;text-align:center" id="iref"></div><div class="row-end" style="margin-top:10px"><a id="idl" href="" download target="_blank" style="padding:8px 14px;background:#1a3252;border:1px solid #2e6da4;border-radius:8px;color:#7eb8f7;text-decoration:none;font-size:13px;font-weight:700">⬇ Yüklə</a><button type="button" onclick="document.getElementById('im').close()">Bağla</button></div></dialog>
<script>function openApprove(id,user,amt){document.getElementById('aid').value=id;document.getElementById('au').textContent=user||'—';document.getElementById('aa').value=(amt&&parseFloat(amt)>0)?amt:'';document.getElementById('am').showModal();}function openReject(id){document.getElementById('rid').value=id;document.getElementById('rm').showModal();}function viewImg(src,ref){document.getElementById('is').src=src;document.getElementById('idl').href=src;document.getElementById('iref').textContent=ref?'Ref: '+ref:'';document.getElementById('im').showModal();}</script>`;
...
async function loadCategories(){
  ...
  grid.innerHTML=categories.map(c=>\`
    <div class="cat-card">
      <img src="\${c.image_url||'/assets/ztopup-logo-gold.png'}" alt="">
      <div class="meta">
...
function updatePreview(){
  ...
  const img=document.getElementById('catImage').value||'/assets/ztopup-logo-gold.png';
